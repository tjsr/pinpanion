...
export const PinnypalsUserImport = (): JSX.Element => {
  ...
  const retrieveLanyard = async (): Promise<void> => {
    ...
    try {
      const data: PinnypalsUserCollectionQuery = await queryPinnypalsUser(config.ppQueryUser, username);
      console.log(data);
      setLoadState('ready');
    } catch (err) {
      setLoadState('error');
      ...
      <FormGroup>
        ...
        <FormControl sx={{ m: 1, minWidth: SEARCH_CONTROL_WIDTH }}>
          <LoadingButton
            ...
            onClick={retrieveLanyard}
          >
            Retrieve
          </LoadingButton>
          <FormHelperText id="pinnypalsLanyardRetrieveButton-helper-text">
            This will query the currently published available and wanted lanyard for the given username and convert it
            to a list displayable in Pinpanion.
          </FormHelperText>
        </FormControl>
      </FormGroup>
    </>
